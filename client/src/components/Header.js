...
export default function Header() {
  ...
  return (
    ...
    <Navbar collapseOnSelect expand="sm" variant="dark" bg={loggedIn && !isHomePage ? "dark" : null}>
      {loggedIn ? (
        <>
          <Navbar.Brand as={Link} to="/" className="brand brand-logged d-flex align-items-center" style={{marginLeft: '20px'}}>
        ...
        (<Navbar.Brand as={Link} to="/" className={`brand brand-new d-flex align-items-center
          ${isLoginPage || isSignupPage ? "brand-text" : null}`} style={{ marginLeft: '20px'}}>
          <img alt="heart" style={{ display: "inline" }} src={heart} className="heart-icon" />
          Exercise Tracker
        </Navbar.Brand>)}
    </Navbar >
